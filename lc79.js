...
// --- BỔ SUNG CẤU HÌNH VIP ENGINE V9.0 ---
const ENABLE_SMART_INVERSION = true; // BẬT chế độ đảo kết quả thông minh chống bẻ cầu của nhà cái
const INVERSION_STRICTNESS = 0.65;   // Ngưỡng kích hoạt đảo cầu tự động khi độ tin cậy bị nhiễu động
...
const VIP_WEIGHTS = {
    ...
    // Trọng số các mẫu VIP mới bổ sung V9.0
...
// --- GIỮ NGUYÊN 100% CÁC THUẬT TOÁN B, S, F, E, G, H, I, J, K, L, M ---
...
// =====================================================================
// === BỘ 5 THUẬT TOÁN BỔ TRỢ CŨ (16 - 20) GIỮ NGUYÊN 100% ===
// =====================================================================
...
// =====================================================================
// === BỘ THUẬT TOÁN BỔ TRỢ SIÊU CẤP MỚI V9.0 (ALGO 21 - ALGO 30) ===
// =====================================================================

// Thuật toán 21: Bẫy Phản Đề Phản Biện - Smart Anti-Reversal
// Mục đích: Phân tích sự chênh lệch kết quả của các thuật toán kinh điển và bẻ ngược lại
function algo21_CounterSystemReversal(history) {
    if (history.length < 15) return null;
    const tx = history.map(h => h.tx);
    const last5 = tx.slice(-5).join('');
    
    // Nếu liên tục xuất hiện nhịp đảo chìm (T-X-T-X) và thuật toán gốc liên tục bị nhiễu
    // AI sẽ dự đoán bẻ ngược xu hướng đám đông để bảo vệ số vốn
    if (last5 === 'TXTXT') return 'T'; // Dự đoán nghịch đảo bẻ cầu 1-1
    if (last5 === 'XTXTX') return 'X';
    return null;
}

// Thuật toán 22: Nhận Diện Cầu Nhảy 1-1 Cương Quyết (Perfect 1-1 Locking)
// Bắt cứng ngắc theo đúng yêu cầu: T-X -> T, X-T -> X liên tục không được lệch
function algo22_StrictAlternating11(history) {
    ...
    // Chuỗi dao động cơ bản của cầu 1-1
...
// Bắt cứng ngắc nhịp X-T-X-X -> T (Cầu 2-2 khuyết nhịp đầu, bù nhịp đầy đủ)
function algo23_StrictDouble22(history) {
    if (history.length < 4) return null;
    const tx = history.map(h => h.tx).slice(-4).join('');
    
    if (tx === 'XTXX') return 'T'; // Khóa chuẩn 2-2 thiếu nhịp đầu -> Lên T
    if (tx === 'TXXT') return 'T'; // Đối xứng chuỗi lót -> Lên T
    if (tx === 'TXTT') return 'X'; // Khóa chuẩn 2-2 thiếu nhịp đầu -> Xuống X
    if (tx === 'XTTX') return 'X'; // Đối xứng chuỗi lót -> Xuống X
    
    // Chuỗi 2-2 cơ bản hoàn hảo
    const last4 = history.map(h => h.tx).slice(-4).join('');
    if (last4 === 'TTXX') return 'T'; // Hết cặp XX bẻ sang cặp TT tiếp theo
    if (last4 === 'XXTT') return 'X'; // Hết cặp TT bẻ sang cặp XX tiếp theo
    
    return null;
}

// Thuật toán 24: Bộ Lọc Động Năng Nến Xúc Xắc Kép (Dual Dice Kinetic Entropy)
// Đọc lực lắc của 3 quân xúc xắc để đoán phản xạ của trọng trường vật lý
function algo24_DualDiceKineticEntropy(history) {
    if (history.length < 5) return null;
    const lastRecord = history[history.length - 1];
    const dice = lastRecord.dice; // [d1, d2, d3]
    const sumDice = lastRecord.total;
    
    // Nếu tổng điểm cực đại (15, 16, 17) nhưng có xúc xắc lẻ rơi vào góc 1 hoặc 2
    // Thể hiện lực nảy cản trở -> Xu hướng giật lùi về Xỉu rất cao
    if (sumDice >= 15 && (dice.includes(1) || dice.includes(2))) {
        return 'X';
    }
    // Nếu tổng điểm cực tiểu (4, 5, 6) nhưng có xúc xắc lớn rơi vào góc 5 hoặc 6
    // Thể hiện lực hồi chuyển mạnh -> Xu hướng bật lên Tài rất cao
...
// Áp dụng lý thuyết sóng và tỷ lệ vàng để nhận diện điểm bão hòa của nhịp Tài/Xỉu
function algo25_FibonacciWavesAnalyzer(history) {
    ...
    // Các điểm rơi Fibonacci cực độ của chuỗi liên tục
    if (lastRun.len === 5) return lastRun.val === 'T' ? 'X' : 'T'; // Sóng 5 hoàn thành, bẻ cầu
    if (lastRun.len === 8) return lastRun.val === 'T' ? 'X' : 'T'; // Sóng 8 cực đại, auto bẻ
    return null;
}

// Thuật toán 26: Chuỗi Markov Inversion Đa Cấp (Multi-stage Inverted Markov)
// Tính toán ma trận chuyển trạng thái nghịch đảo để bắt bài xu hướng bẻ cầu của nhà cái
function algo26_MultiStageInvertedMarkov(history) {
    ...
    // Nếu nhà cái liên tục bẻ ngược kết quả của phân tích Markov thông thường
    // Ta đi trước một bước: Đánh bẻ lại dự đoán của chính Markov gốc!
    if (tCount > xCount && tCount >= 2) return 'X'; // Dự đoán Markov gốc là T -> Ta đánh X
    if (xCount > tCount && xCount >= 2) return 'T'; // Dự đoán Markov gốc là X -> Ta đánh T
    return null;
}

// Thuật toán 27: Phân Tích Độ Biến Động Chu Kỳ Nén (Compression Variance Radar)
// Khi điểm số Tài Xỉu bị nén chặt vào vùng giữa (9, 10, 11, 12), báo hiệu một cú bùng nổ cực mạnh
function algo27_CompressionVarianceRadar(history) {
    ...
    // Độ lệch chuẩn cực thấp = Trạng thái nén chặt
    const stdDev = Math.sqrt(avg(totals.map(t => Math.pow(t - mean, 2))));
    if (stdDev < 1.5) {
        // Cầu nén chặt, bùng nổ theo hướng của phiên cuối cùng
        return last8[7].tx;
    }
    return null;
}
...
// Quét sâu toàn bộ lịch sử 300 phiên để tìm phân đoạn giống hệt nhịp hiện tại
function algo28_DeepSequenceMatching(history) {
    ...
    for (let i = tx.length - 6; i >= 0; i--) {
        ...
        if (sub === target) {
            // Lấy kết quả nghịch đảo của phiên kế tiếp để tối ưu hóa chống bẻ cầu
            return tx[i + 4] === 'T' ? 'X' : 'T';
        }
    }
    return null;
...
// Đo lường điểm rơi của mã hóa MD5 thông qua phân bổ tổng xúc xắc để bắt cầu nghịch hành
function algo29_MD5GravityExtremes(history) {
    if (history.length < 10) return null;
    const last3 = history.slice(-3);
    const sumTotal = last3.reduce((acc, cur) => acc + cur.total, 0);
    
    // Trọng cực Tài cực đại (3 phiên liên tiếp tổng điểm > 40) -> Chắc chắn xả Xỉu
    if (sumTotal >= 41) return 'X';
    // Trọng cực Xỉu cực tiểu (3 phiên liên tiếp tổng điểm < 20) -> Chắc chắn bùng Tài
    if (sumTotal <= 19) return 'T';
    return null;
}

// Thuật toán 30: Vi Long Tuyệt Đối Khóa Chốt (Vi Long Absolute Overlord)
// Quyền lực tối thượng: Tổng hợp lực cầu toàn diện để đưa ra quyết định tối quan trọng bảo vệ người chơi
function algo30_ViLongAbsoluteOverlord(history) {
    if (history.length < 10) return null;
    const tx = history.map(h => h.tx);
    const lastRun = extractFeatures(history).runs.slice(-1)[0];
    
    // Nếu phát hiện cầu bệt dài hơn 6 nhịp -> Bám cứng bệt không bao giờ được bẻ cầu
...
// --- DANH SÁCH THUẬT TOÁN ĐẦY ĐỦ (BỔ SUNG LÊN ĐẾN 30 THUẬT TOÁN) ---
const ALL_ALGS = [
    ...
    // Thuật toán VIP mới V9.0 bổ sung
...
class SEIUEnsemble {
    ...
    predict(history) {
        ...
        // Ưu tiên cao nhất: Không cho phép sai lệch khi cầu đang vào luồng chuẩn
        
        // 1. Khóa cầu 1-1 Tuyệt Đối (T-X -> T, X-T -> X)
        const strict11 = algo22_StrictAlternating11(history);
        if (strict11) {
            // Bộ Lọc Đảo Dự Đoán AI Vi Long
            const finalPrediction = ENABLE_SMART_INVERSION ? (strict11 === 'T' ? 'X' : 'T') : strict11;
            return {
                prediction: finalPrediction === 'T' ? 'Tài' : 'Xỉu',
                confidence: 0.99,
                rawPrediction: finalPrediction,
                note: "🔥 [ULTRA GOD MODE V9.0] KHÓA CỨNG 1-1 TUYỆT ĐỐI " + (ENABLE_SMART_INVERSION ? "(ĐÃ ĐẢO CẦU)" : "")
            };
        }
        ...
        if (strict22) {
            // Bộ Lọc Đảo Dự Đoán AI Vi Long
            const finalPrediction = ENABLE_SMART_INVERSION ? (strict22 === 'T' ? 'X' : 'T') : strict22;
            return {
                prediction: finalPrediction === 'T' ? 'Tài' : 'Xỉu',
                confidence: 0.98,
                rawPrediction: finalPrediction,
                note: "🔥 [ULTRA GOD MODE V9.0] KHÓA CỨNG 2-2 TOÀN DIỆN " + (ENABLE_SMART_INVERSION ? "(ĐÃ ĐẢO CẦU)" : "")
            };
        }
        ...
        if (flashOverride) {
            const finalPrediction = ENABLE_SMART_INVERSION ? (flashOverride === 'T' ? 'X' : 'T') : flashOverride;
            return {
                prediction: finalPrediction === 'T' ? 'Tài' : 'Xỉu',
                confidence: 0.99, 
                rawPrediction: finalPrediction,
                note: "🔥 [ULTRA GOD MODE] KHÓA CỨNG CẦU 1-1 / 2-2 " + (ENABLE_SMART_INVERSION ? "(ĐÃ ĐẢO CẦU)" : "")
            };
        }
        ...
        for (const a of this.algs) {
            try {
                ...
                // Thuật toán V9.0 mới bổ sung boost vượt bậc
        ...
        if (votes.T === 0 && votes.X === 0) {
            ...
            const finalFallback = ENABLE_SMART_INVERSION ? (fallback === 'T' ? 'X' : 'T') : fallback;
            return { prediction: finalFallback === 'T' ? 'Tài' : 'Xỉu', confidence: 0.5, rawPrediction: finalFallback };
        }
        ...
        // --- QUY TRÌNH CHẠY BỘ LỌC ĐẢO DỰ ĐOÁN AI VI LONG ---
        if (isBet) {
             const betPred = lastRun.val;
             // Cầu bệt cực dài thì không đảo để tránh cháy tài khoản
             const finalPred = betPred; 
             return {
        ...
        if (is1_1) {
             ...
             const finalPred = ENABLE_SMART_INVERSION ? (nextPred === 'T' ? 'X' : 'T') : nextPred;
             return {
                 prediction: finalPred === 'T' ? 'Tài' : 'Xỉu',
                 confidence: Math.max(confidence, 0.95), 
                 rawPrediction: finalPred,
                 note: ENABLE_SMART_INVERSION ? "🔥 ĐÃ ĐẢO CẦU 1-1 CHỐNG BẼ" : ""
             };
        }
        ...
        if (is2_2 || features.runs.slice(-3).map(r=>r.len).join(',') === '2,2,1') {
             ...
             return {
                 ...
                 rawPrediction: finalPred,
                 note: ENABLE_SMART_INVERSION ? "🔥 ĐÃ ĐẢO CẦU 2-2 CHỐNG BẼ" : ""
             };
        }
        ...
        // Đảo kết quả chung cuộc của thuật toán dựa trên quyết định hệ thống
        let finalConsensus = best;
        let activeInversion = false;
        
        if (ENABLE_SMART_INVERSION) {
            // Đảo ngược kết quả: Nếu AI tính ra T thì báo X, ngược lại tính ra X thì báo T
            finalConsensus = best === 'T' ? 'X' : 'T';
            activeInversion = true;
        }
        
        return {
            prediction: finalConsensus === 'T' ? 'Tài' : 'Xỉu',
            confidence: confidence,
            rawPrediction: finalConsensus,
            note: activeInversion ? "⚠️ [HỆ THỐNG ĐÃ ĐẢO NGƯỢC KẾT QUẢ AI VI LONG CHỐNG NHÀ CÁI]" : ""
        };
    }
}
...
app.get("/", async () => { 
    return {
        ...
        version: "9.0 AI Vi Long Absolute Core with Smart Inversion (Anti-Crash)", 
        algorithms: ALL_ALGS.length,
        pattern_recognition: "ULTRA VIP (Khóa cứng 1-1 & 2-2, Đảo Ngược Kết Quả Chống Bẻ Cầu, Anti-Hang)",
        endpoints: [
            "/api/taixiumd5/lc79",
...
const start = async () => {
    ...
    console.log("\n🚀 AI Tài Xỉu MD5 Pro V9.0 - ULTRA GOD MODE (ANTI-CRASH) Đã Khởi Động!");
    ...
    console.log(`\n🔧 Hệ thống AI VIP với ${ALL_ALGS.length} thuật toán nguyên bản và bổ trợ nâng cao:`);
